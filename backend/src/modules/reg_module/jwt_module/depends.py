...
def _validate_access_token_payload(payload: dict) -> None:
    """
    ...
    # validate token_type
    if (
        payload[auth_config.jwt.token_type_field]
        != jwt_schemas.TokenType.access_token.value
    ):
        raise HTTPException(
...
def get_user_id_from_refresh_token(
    ...
    try:
        payload: dict = jwt.decode(
            refresh_token,
            auth_config.jwt.public_key_path,
